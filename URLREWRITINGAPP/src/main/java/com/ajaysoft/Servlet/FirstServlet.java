...
public class FirstServlet extends HttpServlet {
	...
	out.println("<form method='post' action='" + response.encodeURL("./second") + "' class='space-y-4'>");
	out.println("<div>");
	out.println("<label class='block text-sm mb-1 font-semibold text-white'>Account Type</label>");
	out.println("<input type='text' name='accType' placeholder='Enter account type' class='w-full p-2 rounded-lg border border-white/30 bg-white/10 text-white focus:outline-none focus:ring-2 focus:ring-yellow-300'/>");
	out.println("</div>");

	out.println("<div>");
	out.println("<label class='block text-sm mb-1 font-semibold text-white'>Account Branch</label>");
	out.println("<input type='text' name='accBranch' placeholder='Enter branch name' class='w-full p-2 rounded-lg border border-white/30 bg-white/10 text-white focus:outline-none focus:ring-2 focus:ring-yellow-300'/>");
	out.println("</div>");
